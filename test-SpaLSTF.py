...
from model.SpaLSTF_model import SpaLSTF
from model.SpaLSTF_scheduler import NoiseScheduler
from model.SpaLSTF_train import normal_train_SpaLSTF
from model.sample import sample_SpaLSTF
from process.result_analysis import clustering_metrics, get_N_clusters
...
parser.add_argument("--sc_data", type=str, default="dataset1_seq_33.h5ad")
parser.add_argument("--sp_data", type=str, default='dataset1_spatial_33.h5ad')
parser.add_argument("--document", type=str, default='dataset1')
parser.add_argument("--batch_size", type=int, default=2048)  # >512--512    <512--2048
parser.add_argument("--hidden_size", type=int, default=512) # >512--1024    <512--512
...
if adata_seq.shape[1] < 40:
    fix = False
else:
    fix = True
adata_seq2 = data_augment(adata_seq.copy(), fix, noise_std=args.noise_std)
...
sc_genes = np.array(adata_seq.var_names)
...
def impute():
    ...
    for train_ind, test_ind in kf.split(raw_shared_gene):
        ...
        save_path = 'SpaLSTF-ckpt/' + doc + '/'
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        save_path_prefix = save_path + 'SpaLSTF%d.pt' % (idx)
        ...
        # sample
        model.eval()
        imputation = sample_SpaLSTF(model,
                                    device=device,
                                    dataloader=dataloader,
...
Spa_result = impute() # ok
Spa_result_pd = pd.DataFrame(Spa_result, columns=sp_genes)
Spa_result_pd.to_csv(outdir +  '/SpaLSTF_impute.csv',header = 1, index = 1)
