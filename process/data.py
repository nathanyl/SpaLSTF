...
def plot_hvg_umap(hvg_adata,color=['celltype'],path = None, save_filename=None):
    ...
    if save_filename:
        ...
        # save = '.pdf'
        save = f'{save_filename}.png'
    else:
        save = None
    # ideal gas equation

    # if save:
    #     plt.savefig(save, format='tiff', dpi=600)
...
def data_augment(adata: AnnData, fixed: bool, noise_std):
    noise_stddev = noise_std
    augmented_adata = adata.copy()
    augmented_adata_1 = adata.copy()
    augmented_adata_2 = adata.copy()
    gene_expression = adata.X

    augmented_adata_1.X = augmented_adata.X + np.full(gene_expression.shape, noise_stddev * 0.5)
    augmented_adata_2.X = augmented_adata.X + np.full(gene_expression.shape, noise_stddev)
    if fixed:
        # merge_adata = adata.concatenate(augmented_adata, join='outer')
        merge_adata = adata.concatenate(augmented_adata_1, join='outer')
        merge_adata = merge_adata.concatenate(augmented_adata_2, join='outer')
    else:
        merge_adata = adata.concatenate(augmented_adata_2, join='outer')
