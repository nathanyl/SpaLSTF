...
def model_sample_SpaLSTF(model, device, dataloader, total_sample, time, is_condi, condi_flag, edge_index):
    noise = []
    i = 0
    for _, x_cond in dataloader: 
        ...
        if not is_condi:
            n = model(total_sample[i:i+len(x_cond)], t, None, edge_index)
        else:
            n = model(total_sample[i:i+len(x_cond)], t, x_cond, edge_index, condi_flag=condi_flag)
        noise.append(n)
        i = i+len(x_cond)
    noise = torch.cat(noise, dim=0)
    return noise

def sample_SpaLSTF(model,
                dataloader,
                noise_scheduler,
                mask = None,
                gt = None,
    ...
    for t_idx, time in enumerate(ts):
        ...
        with torch.no_grad():
            ...
            model_output = model_sample_SpaLSTF(model,
                                        device=device,
            ...
            if is_classifier_guidance:
                model_output_uncondi = model_sample_SpaLSTF(model,
                                                    device=device,
                                                    dataloader=dataloader,
                                                    total_sample=sample,
                                                    time=time,
